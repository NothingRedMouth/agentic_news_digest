...
<<<<<<< HEAD
        ...
        if not campaign_res["success"]:
            return {
                "success": False,
                "error": f"Ошибка на шаге createCampaign: {campaign_res.get('error')}",
            }
...
<<<<<<< HEAD
                            ...
                            logger.error(
                                f"Ошибка выполнения задачи экспорта. Статус: {status}"
                            )
                            return []
                except Exception as e:
                    logger.error(
                        f"Исключение при проверке статуса задачи {task_uuid}: {e}"
                    )
                    return []
...
<<<<<<< HEAD
                    ...
                    header = lines[0]
                    delimiter = (
                        ";" if ";" in header else ("," if "," in header else "\t")
                    )
